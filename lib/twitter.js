...
Twitter.prototype.stream = function(method, params, callback) {
	...
	stream.destroy = function() {
		...
		stream.emit('destroy','Socket destroyed');
		clearInterval(stream.hb_interval); // Clear heartbeat interval.
	};
	...
	request.on('response', function(response) {
		...
		if(response.statusCode == 200) {
			stream.emit('connect_success', response);
		}

		// Some kind of error connecting
		if(response.statusCode > 200) {
			stream.emit('connect_fail',response.statusCode);
			stream.destroy();
		}
	...
	// Check to see if we're still getting data over the wire from twitter.
	stream.check_heartbeat();
